...
<html>
<?php
require_once 'orderdetailscontrol.php';
?>
<head>
  <meta charset="utf-8">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <form method="post">
        ...
        <div class="box-body">
          <div class="col-sm-6">
            <label class="pull-left">Order Status
                <select name="chgstatus">
                    <option value="0" <?php if($status[0]->OrderStatus==0) echo "Selected" ?>>Placed</option>
                    <option value="1" <?php if($status[0]->OrderStatus==1) echo "Selected" ?>>Confirmed</option>
                    <option value="2" <?php if($status[0]->OrderStatus==2) echo "Selected" ?>>Canceled</option>
                    <option value="3" <?php if($status[0]->OrderStatus==3) echo "Selected" ?>>Dispatched</option>
                    <option value="4" <?php if($status[0]->OrderStatus==4) echo "Selected" ?>>Not Deliverd</option>
                    <option value="5" <?php if($status[0]->OrderStatus==5) echo "Selected" ?>>Delivered</option>
                </select>
            </label>
          </div>
          <div class="col-sm-6">
            <label class="pull-right">Return Status:
            <?php 
              if(isset($rstatus))
                { 
                  if($rstatus[0]->ReturnStatus==0)
                  {
                    echo "Not Requested";
                  }
                  elseif($rstatus[0]->ReturnStatus==1)
                  {
                    echo "Requested Return";
                  } 
                  elseif($rstatus[0]->ReturnStatus==2)
                  {
                    echo "Return Confirmed/Return Processing";
                  }
                  elseif($rstatus[0]->ReturnStatus==3)
                  {
                    echo "Returned";
                  }
                } 
              else
               echo "Not Requested"; ?></label>
          </div>
        </div>
        <!-- /.box-body -->
      ...
      <div class="box box-primary">
        ...
        <div class="box-body">
          <div class="col-sm-6">
              <div class="col-sm-12">
                  OrderID:<label><?php echo $status[0]->OrderID ?></label>
              </div>
              <div class="col-sm-12">
          ...
          <table class="table table-bordered table-striped">
            <tbody>
                ...
                <tr>
                  ...
                  <td><?php echo $p->SizeXQuantity ?></td>
                  <td><?php echo $p->OrderQuantity; ?></td>
                  <td><?php echo $p->SellingPrice*$p->OrderQuantity; $sum=$sum+$p->SellingPrice*$p->OrderQuantity; ?></td>
                </tr>
                <?php } ?>
            </tbody>
            <tfoot>
